...
#import "UIView+RFDAlertView.h"
#import "AppDelegate.h"
#import <QuartzCore/QuartzCore.h>
@implementation UIView (RFDAlertView)
...
+ (void) addRFDNotifierWithText : (NSString* ) text dismissAutomatically : (BOOL) shouldDismiss {
    ...
    if(notifierView) {
        ...
        if(shouldDismiss) {
            [self performSelector:@selector(dismissRFDNotifier) withObject:nil afterDelay:2.0];
        }
    }
    else {
...
+ (UIView* ) checkIfNotifierExistsAlready {
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(dismissRFDNotifier) object:nil];
